...
function form(){
	...
	self.list = function(scope) {
		...
		$('#x_content').load('lists/supply.html', function() {
	...
	function validate(scope) {
									// change
		var controls = scope.formHolder.supply.$$controls;
		...
		angular.forEach(controls,function(elem,i) {
			...
			if (elem.$$attr.$attr.required) elem.$touched = elem.$invalid;
							 ...
							 // change
		return scope.formHolder.supply.$invalid;
	...
	self.addEdit = function(scope,row) {	
		...
		$('#x_content').load('forms/supply.html',function() {
		...
		if (row != null) {
			...
			$http({
			  method: 'POST',
			  url: 'handlers/supply/view.php',
			  data: {id: row.id}
			}).then(function mySucces(response) {
				
				angular.copy(response.data, scope.supply);
	...
	self.save = function(scope) {
		...
		$http({
		  method: 'POST',
		  url: 'handlers/supply/save.php',
		data: {supply: scope.supply}
		}).then(function mySucces(response) {
			
			if (scope.supply.id == 0) {
				scope.supply.id = response.data;
	...
	self.delete = function(scope,row) {
		...
		var onOk = function() {
			...
			$http({
			  method: 'POST',
			  url: 'handlers/supply/delete.php',
			  data: {id: [row.id]}
			}).then(function mySucces(response) {
