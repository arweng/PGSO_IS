...
function form(){
	...
	self.list = function(scope) {
		...
		$('#x_content').load('lists/brand.html', function() {
	...
	function validate(scope) {
									// change
		var controls = scope.formHolder.brand.$$controls;
		...
		angular.forEach(controls,function(elem,i) {
			...
			if (elem.$$attr.$attr.required) elem.$touched = elem.$invalid;
							 ...
							 // change
		return scope.formHolder.brand.$invalid;
	...
	self.addEdit = function(scope,row) {	
		...
		$('#x_content').load('forms/brand.html',function() {
	...
	self.save = function(scope) {
		...
		$http({
		  method: 'POST',
		  url: 'handlers/brand/save.php',
		data: {brand: scope.brand}
		}).then(function mySucces(response) {
			
			if (scope.brand.id == 0) {
				scope.brand.id = response.data;
				growl.show('btn btn-success',{from: 'top', amount: 55},'Brand Information successfully added.');
				}	else{
					growl.show('btn btn-success',{from: 'top', amount: 55},'Brand Information successfully updated.');
				}
			mode(scope,scope.brand);
